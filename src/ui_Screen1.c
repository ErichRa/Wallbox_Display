...
void ui_Screen1_screen_init(void)
{
    ...
    ui_HumiLabel = make_label(main_panel, "BEREIT", 210, 87,
                              &lv_font_montserrat_20, COLOR_GREEN);
    lv_obj_set_width(ui_HumiLabel, 316);
    lv_label_set_long_mode(ui_HumiLabel, LV_LABEL_LONG_CLIP);
    ...
    ui_TotalEnergyLabel = make_label(footer, "Energie gesamt: --.-- kWh", 14, 8,
                                     &lv_font_montserrat_14, COLOR_TEXT);
    ui_ClockLabel = make_label(footer, "--:--", 600, 7,
                               &lv_font_montserrat_16, COLOR_TEXT);
    lv_obj_set_width(ui_ClockLabel, 138);
    lv_obj_set_style_text_align(ui_ClockLabel, LV_TEXT_ALIGN_RIGHT, 0);
}
